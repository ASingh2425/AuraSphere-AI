...
export default function OpsCopilot({ onCopilotAction }) {
  ...
  const executeCommand = (cmdText) => {
    ...
    // Call the streaming response engine with type 'ops'
    simulateStreamingResponse(cmdText, 'ops', (chunk) => {
      // Set reasoning logs if returned
      if (chunk.reasoning) {
        setReasoningLogs(chunk.reasoning);
      }
      
      // Update typewriter response
      ...
      if (chunk.done) {
        ...
        if (chunk.actionItem) {
          ...
          // Execute callback to update 3D map modes
  ...
  const handleBroadcast = () => {
    ...
    // 1. Synthesize audio chime alert
    try {
    ...
    // 2. Synthesize spoken voice notification in target language
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel(); // Terminate any lingering queues
      ...
      utterance.rate = 0.92; // Robotic pacing
      utterance.pitch = 1.0;
  ...
  return (
    <div className="glass-panel" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: '400px', padding: '16px' }}>
